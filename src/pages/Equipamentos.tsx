...
type Equipamento = {
  ...
  origem?: string | null; proprietario?: string | null; quantidade?: number | null;
  estado?: string | null; foto_url?: string | null; valor_estimado?: number | null;
  observacoes?: string | null; created_at?: string | null;
};

type EmprestimoAtivo = {
  id: string; equipamento_id: string | null; responsavel_nome: string | null;
  data_retirada: string | null; estado_saida: string | null; foto_saida_url: string | null;
};
...
const Equipamentos = () => {
  ...
  const { data: emprestimosAtivos = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("equipamento_emprestimos").select("id, equipamento_id, responsavel_nome, data_retirada, estado_saida, foto_saida_url").eq("devolvido", false);
      if (error) throw error;
    ...
    <div className="space-y-4 md:space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
          {catCards.map((c) => (
            <Card key={c.cat} className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => setFiltroCategoria(filtroCategoria === c.cat ? "todos" : c.cat)}>
              <CardHeader className="pb-2 pt-4 px-4"><CardTitle className="text-sm font-medium text-muted-foreground">{c.cat}</CardTitle></CardHeader>
              <CardContent className="px-4 pb-4"><p className="text-2xl font-bold">{c.total}</p><p className="text-xs text-muted-foreground">R$ {c.valor.toFixed(2)}</p></CardContent>
            </Card>
          ))}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <Select value={filtroOrigem} onValueChange={setFiltroOrigem}>
          ...
          <SelectContent>{ORIGENS_FILTER.map((o) => <SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>)}</SelectContent>
        </Select>
        <Select value={filtroEstado} onValueChange={setFiltroEstado}>
          <SelectTrigger className="w-full"><SelectValue /></SelectTrigger>
          <SelectContent>{ESTADOS_FILTER.map((e) => <SelectItem key={e.value} value={e.value}>{e.label}</SelectItem>)}</SelectContent>
        </Select>
      </div>
      ...
      {isMobile ? (
        <div className="space-y-3">
          {filtered.length === 0 ? (
            <p className="text-center py-8 text-muted-foreground">Nenhum equipamento encontrado</p>
          ) : filtered.map((eq) => {
              ...
              <Card key={eq.id}>
                <CardContent className="p-3 space-y-2">
                  <div className="flex gap-3 items-start">
                    {eq.foto_url ? (
                      <img src={eq.foto_url} alt="" className="w-12 h-12 rounded object-cover shrink-0" />
                    ) : (
                      <div className="w-12 h-12 rounded bg-muted flex items-center justify-center shrink-0"><Wrench className="h-4 w-4 text-muted-foreground" /></div>
                    )}
                    <div className="flex-1 min-w-0">
                      <p className="font-medium truncate">{eq.nome}</p>
                      <div className="flex gap-2 flex-wrap mt-1">
                        <Badge variant="outline">{eq.categoria}</Badge>
                        <Badge variant={eq.estado === "Bom" ? "secondary" : eq.estado === "Danificado" ? "destructive" : "outline"}>{eq.estado}</Badge>
                      </div>
                    </div>
                  </div>
                  <div className="text-sm text-muted-foreground grid grid-cols-2 gap-1">
                    <span>Qtd: {eq.quantidade}</span>
                    <span>{origemLabel[eq.origem || "proprio"]}</span>
                  </div>
                  <div>
                    {emprestimo ? (
            ...
            <TableBody>
              {filtered.length === 0 ? (
                ...
                return (
                  <TableRow key={eq.id}>
                    <TableCell>
                      {eq.foto_url ? (<img src={eq.foto_url} alt="" className="w-10 h-10 rounded object-cover" />) : (<div className="w-10 h-10 rounded bg-muted flex items-center justify-center"><Wrench className="h-4 w-4 text-muted-foreground" /></div>)}
                    </TableCell>
                    ...
                    <TableCell><Badge variant={eq.estado === "Bom" ? "secondary" : eq.estado === "Danificado" ? "destructive" : "outline"}>{eq.estado}</Badge></TableCell>
                    <TableCell>
                      {emprestimo ? (<Badge className="bg-orange-500 text-white hover:bg-orange-600">Em uso por {emprestimo.responsavel_nome}</Badge>) : (<Badge className="bg-green-600 text-white hover:bg-green-700">Disponível</Badge>)}
                    </TableCell>
                    <TableCell>
      ...
      {emprestarEquip && <EmprestarDialog open={emprestarOpen} onOpenChange={setEmprestarOpen} equipamentoId={emprestarEquip.id} equipamentoNome={emprestarEquip.nome} onSuccess={refetch} />}
      {devolverData && <DevolverDialog open={devolverOpen} onOpenChange={setDevolverOpen} emprestimo={devolverData.emprestimo} equipamentoNome={devolverData.nome} onSuccess={refetch} />}
      {detalhesEquip && <EquipamentoDetalhesDialog open={detalhesOpen} onOpenChange={setDetalhesOpen} equipamento={detalhesEquip} />}
    </div>
  );
};
