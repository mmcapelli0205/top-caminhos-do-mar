...
const ReceitaSection = () => {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Doações */}
      <Collapsible open={openDoacoes} onOpenChange={setOpenDoacoes}>
        <CollapsibleTrigger asChild>
          <button className="w-full flex items-center justify-between p-3 rounded-lg border bg-card hover:bg-muted/50 transition-colors">
            <div className="flex items-center gap-3">
              <h3 className="text-lg font-semibold">Doações</h3>
              <span className="text-sm text-muted-foreground">({(doacoes ?? []).length} doações)</span>
            </div>
            <div className="flex items-center gap-3">
              <span className="font-bold">{fmt(totalDoacoes)}</span>
              <ChevronDown className={`h-5 w-5 text-muted-foreground transition-transform ${openDoacoes ? "rotate-180" : ""}`} />
            </div>
          </button>
        </CollapsibleTrigger>
        <CollapsibleContent className="space-y-4 pt-4">
